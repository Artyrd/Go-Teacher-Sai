...
import uuid
import global_var
from game_logic import game_logic, stable_board_state, increment_exp_ai_col, increment_exp_player_col, turn_colour
...
def show_wait_destroy(winname, img):
    cv2.imshow(winname, rotate_image(img))
...
def evaluate_board_state(src_file, mode):
    print("Evaluating board state...")
    board_size = 19
    evaluation_map = {}
    # open image
    ...
    board_img = cv2.imread(r'result\flatboard.jpg')
    # Check if image is loaded fine
    if src is None:
        print ('Error opening image: ' + squareFile)
        return -1
    black_count = 0
    white_count = 0
    # cut up images to analyse each intersection
    for coord in coordinates:
        x,y = coord
        eval_width = 12
        ...
        # determine if intersection contains 'black', 'white', or 'board'
        ...
        if color == "black":
            cv2.circle(board_img, coord, 15, (0, 0, 0), -1)
            black_count += 1
        if color == "white":
            cv2.circle(board_img, coord, 15, (248,248,255), -1)
            white_count += 1
    #print(f"black stones = {black_count}, white stones = {white_count}")
    
    # display capture images for test capture or calibrating resume game
    if mode == 1 or mode == 2:
        show_wait_destroy("evaluation_area", display_img)
        show_wait_destroy("calculated state", board_img)
        if mode == 1:
            global_var.b_exp = black_count
            global_var.w_exp = white_count
        return

    if not stable_board_state(black_count, white_count):
        return False
    else:
        print("captured image is stable")

    logic_result = game_logic(black_count, white_count)
    print(logic_result)
    if logic_result:
        print("Legal move has been registered")
        global_var.turn += 1
    else:
        return False

    #if board state updated to ai's move
    if turn_colour() == global_var.player_colour:
        # wait for human move
        global_var.status = 'green'
        #global_var.turn += 1
        print("incrementing player col")
        increment_exp_player_col()
        return False
    '''
    else:
            filename2 = r"result\response_move.jpg"
            cv2.imwrite(filename2, rotate_image(board_img))
    '''
    ...
    # AI's TURN: 
    print("calling save_SGF for current board state...")
    next_move = save_SGF(board)
    print("response: ",next_move)
    #global_var.turn += 1
    global_var.status = 'red'
    print("incrementing ai col")
    increment_exp_ai_col()
    
    #next_move formated as "w, (16,4)"
    x, y = next_move[1]
    color = next_move[0]
    response_coord = matrix[(board_size-1) - x][y]
    # cv2 draws on 'emptyboard'
    if color == "b":
            cv2.circle(board_img, response_coord, 15, (0, 0, 0), -1)
            cv2.circle(board_img, response_coord, 9, (255, 255, 255), 2)
    if color == "w":
        cv2.circle(board_img, response_coord, 15, (255, 255, 255), -1)
        cv2.circle(board_img, response_coord, 9, (0, 0, 0), 2)
    #show_wait_destroy("gnugo response!", board_img)
    
    # saving response to jpg for display
    filename = r"result\response_move.jpg"
    cv2.imwrite(filename, rotate_image(board_img))
